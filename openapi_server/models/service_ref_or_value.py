...
SERVICEREFORVALUE_ONE_OF_SCHEMAS = ["Service", "ServiceRef"]
...
class ServiceRefOrValue(BaseModel):
    ...
    oneof_schema_2_validator: Optional[ServiceRef] = None
    actual_instance: Optional[Union[Service, ServiceRef]] = None
    one_of_schemas: List[str] = Literal["Service", "ServiceRef"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ServiceRefOrValue.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, Service):
            ...
            match += 1
        # validate data type: ServiceRef
        if not isinstance(v, ServiceRef):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ServiceRef`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ServiceRefOrValue with oneOf schemas: Service, ServiceRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ServiceRefOrValue with oneOf schemas: Service, ServiceRef. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ServiceRef
        try:
            instance.actual_instance = ServiceRef.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ServiceRefOrValue with oneOf schemas: Service, ServiceRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ServiceRefOrValue with oneOf schemas: Service, ServiceRef. Details: " + ", ".join(error_messages))
        else:
            return instance
...
ServiceRefOrValue.model_rebuild(raise_errors=False)
