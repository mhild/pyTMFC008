...
ENTITYREFORVALUE_ONE_OF_SCHEMAS = ["Entity", "EntityRef"]

class EntityRefOrValue(BaseModel):
    """
    ...
    actual_instance: Optional[Union[Entity, EntityRef]] = None
    one_of_schemas: List[str] = Literal["Entity", "EntityRef"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = EntityRefOrValue.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in EntityRefOrValue with oneOf schemas: Entity, EntityRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in EntityRefOrValue with oneOf schemas: Entity, EntityRef. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into EntityRefOrValue with oneOf schemas: Entity, EntityRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into EntityRefOrValue with oneOf schemas: Entity, EntityRef. Details: " + ", ".join(error_messages))
        else:
            return instance
