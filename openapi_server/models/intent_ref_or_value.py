...
INTENTREFORVALUE_ONE_OF_SCHEMAS = ["Intent", "IntentRef"]
...
class IntentRefOrValue(BaseModel):
    ...
    """
    # data type: IntentRef
    oneof_schema_1_validator: Optional[IntentRef] = None
    # data type: Intent
    oneof_schema_2_validator: Optional[Intent] = None
    actual_instance: Optional[Union[Intent, IntentRef]] = None
    one_of_schemas: List[str] = Literal["Intent", "IntentRef"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = IntentRefOrValue.model_construct()
        error_messages = []
        match = 0
        # validate data type: IntentRef
        if not isinstance(v, IntentRef):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IntentRef`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in IntentRefOrValue with oneOf schemas: Intent, IntentRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in IntentRefOrValue with oneOf schemas: Intent, IntentRef. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into IntentRef
        try:
            instance.actual_instance = IntentRef.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into Intent
        try:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into IntentRefOrValue with oneOf schemas: Intent, IntentRef. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into IntentRefOrValue with oneOf schemas: Intent, IntentRef. Details: " + ", ".join(error_messages))
        else:
            return instance
